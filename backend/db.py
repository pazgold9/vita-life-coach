...
def update_profile(updates: dict, profile_id: str = DEFAULT_PROFILE_ID) -> bool:
    ...
    try:
        ...
        clean["id"] = profile_id
        client.table(PROFILES_TABLE).upsert(clean).execute()
        return True
    except Exception as e:
...
def reset_profile(profile_id: str = DEFAULT_PROFILE_ID) -> bool:
    """Clear all profile fields so the next visitor starts fresh."""
    client = _get_client()
    if client is None:
        return False
    try:
        blank = {field: None for field in PROFILE_FIELDS}
        blank["updated_at"] = datetime.now(timezone.utc).isoformat()
        client.table(PROFILES_TABLE).update(blank).eq("id", profile_id).execute()
        return True
    except Exception as e:
        logger.warning("Failed to reset profile: %s", e)
        return False


def get_profile_completeness(profile_id: str = DEFAULT_PROFILE_ID) -> tuple[bool, list[str]]:
    """Check if all required profile fields are filled. Returns (is_complete, missing_fields)."""
